...
export class DexClient {
  ...
  async createHTLCWithSwap(
    ...
    const result = await (this.cosmosClient as any).execute(
      sender,
  ...
  async querySpotPrice(
    ...
    const response = await (this.cosmosClient as any).queryContractSmart(
      this.htlcContractAddress,
      query
    );
        ...
        try {
          ...
          if (routeInfo.routes.length > 1) {
            ...
            if (priceDiff > 0.005) { // 0.5% arbitrage opportunity
              callback({
                tokenPair: pair,
                buyRoute: sortedPrices[0].route,
                sellRoute: sortedPrices[sortedPrices.length - 1].route,
                profitPercentage: (priceDiff * 100).toFixed(2),
                prices: sortedPrices,
              });
            }
          }
